...
void G1(double *x,double *y,double *u,double *v,int j);
...
int main(void){
	...
	while(t<=T){
		...
		if(j_current < 0){//DWC
			G1(&x[i_current],&y[i_current],&u[i_current],&v[i_current],j_current);
			if(j_current == -3){
		...
		if((((j_current == -1)||(j_current == -2))&&(t > Trec))&&(y[i_current] > h_rec)){//DWC
			fprintf(fp_hist,"%lf %lf %lf %lf %d %d\n",x[i_current],y[i_current],u[i_current],v[i_current],i_current,j_current);
		}
